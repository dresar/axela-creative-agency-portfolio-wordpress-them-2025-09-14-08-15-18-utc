		...
		<?php if(has_post_thumbnail()): ?>
			...
			?>
				<div class="sale--box">
					...
					$is_new = axela_is_recent_post();
					if ($is_new){
						?> <span class="new"><?php esc_html_e( 'NEW', 'axela' );?></span>  <?php
					}
					if( $is_feat ){
						?> <span class="hot">
							<?php esc_html_e( 'HOT', 'axela' ); ?> </span>  <?php
					}
					?>
				</div>
				<div class="quick-wish">
					<?php if ( function_exists( 'YITH_WCQV_Frontend' ) && $axela_option['wc_quickview_icon']  ): ?>
						<div class="axela-quick">					
							<a href="" class="yith-wcqv-button" data-product_id="<?php echo esc_attr( $product->get_id() );?>"><i class="far fa-eye"></i></a>
						</div>
					<?php endif; ?>
					
						<?php if ( class_exists( 'YITH_WCWL_Shortcode' ) && $axela_option['wc_wishlist_icon'] ): ?>
							<div class="axela-wishlist">
							<?php
								$args = array(
