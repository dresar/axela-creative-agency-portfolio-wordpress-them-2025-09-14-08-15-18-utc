<?php
   ...
   if (is_home() && !is_front_page() || is_home() && is_front_page()){
    ...
    if ($header_width_meta != ''){
        ...
        $header_width = !empty($axela_option['header_grid2']) ? $axela_option['header_grid2'] : '';
...
elseif( !empty($axela_option['footer_style'])){
    $footer_style = $axela_option['footer_style'];
    $footer_style = new WP_Query(array(
        'post_type' => 'rts-footer',
        'posts_per_page' => -1,
        'p' => $axela_option['footer_style'],
    )); 
